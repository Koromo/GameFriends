...
//------------------------------------------------------------------------------------------------
//すべての配列に (たとえば GetCopyableFootprints を呼び出すことにより) 値を入れる必要があります
...
//------------------------------------------------------------------------------------------------
// ヒープを割り振る UpdateSubresources の実装
inline UINT64 UpdateSubresources( 
    _In_ ID3D12GraphicsCommandList* pCmdList,
    _In_ ID3D12Resource* pDestinationResource,
    _In_ ID3D12Resource* pIntermediate,
...
//------------------------------------------------------------------------------------------------
// スタックを割り振る UpdateSubresources の実装
...
{
    //このキャストは、厳密に型指定されたコマンド リスト ポインターを以下に渡すのに役立ちます: 
    // ExecuteCommandLists。
    //このキャストは、const 性が守られる限り有効です。D3D12 API では、
    //引数の const 性が適用されます。
